...
def mock_frm():
    ...
    frm.is_online = True
    frm.last_error = ""
    # Make all data methods return AsyncMock
    ...
    frm.get_doggos = AsyncMock(return_value=[])
    return frm
...
class TestCmdHelp:
    ...
    async def test_help_lists_commands(self, handler):
        """Test help lists all available commands."""
        result = await handler.cmd_help("")
...
class TestCmdGraph:
    ...
    def mock_grafana(self):
        ...
        grafana.render_panel = AsyncMock(return_value=None)
        return grafana
    ...
    async def test_graph_not_configured(self, handler):
        """Test graph when Grafana not configured."""
        result = await handler.cmd_graph("")
        assert "not configured" in result.lower()

    async def test_graph_list_no_args(self, handler_with_grafana, mock_grafana):
        """Test graph with no args lists available panels."""
        result = await handler_with_grafana.cmd_graph("")
        assert "Available graphs:" in result
        assert "power" in result
        assert "production" in result
        assert "electricity" in result

    async def test_graph_list_explicit(self, handler_with_grafana):
        """Test graph list subcommand."""
        result = await handler_with_grafana.cmd_graph("list")
        assert "Available graphs:" in result

    async def test_graph_render_success(self, handler_with_grafana, mock_grafana):
        """Test successful graph render returns ImageResponse."""
        mock_grafana.render_panel.return_value = b"fake-png-data"

        result = await handler_with_grafana.cmd_graph("power")
    ...
    async def test_graph_render_with_time_range(self, handler_with_grafana, mock_grafana):
        """Test graph render with custom time range."""
        mock_grafana.render_panel.return_value = b"fake-png-data"

        result = await handler_with_grafana.cmd_graph("power 24h")

        assert isinstance(result, ImageResponse)
        assert "24h" in result.caption
        mock_grafana.render_panel.assert_called_once_with("power", time_range="24h")

    async def test_graph_unknown_panel(self, handler_with_grafana, mock_grafana):
        """Test graph with unknown panel name."""
        result = await handler_with_grafana.cmd_graph("nonexistent")

        assert "Unknown graph" in result
        assert "power" in result  # Should suggest available panels

    async def test_graph_render_failure(self, handler_with_grafana, mock_grafana):
    ...
    async def test_graph_no_panels_configured(self, mock_frm, config, mock_server):
    ...
    async def test_graph_via_handle(self, handler_with_grafana, mock_grafana):
        """Test graph command through main handle method."""
        mock_grafana.render_panel.return_value = b"image-data"

        result = await handler_with_grafana.handle("/graph power")

        assert isinstance(result, ImageResponse)

    async def test_help_includes_graph_when_configured(self, handler_with_grafana):
        """Test help output includes graph command when Grafana is configured."""
        result = await handler_with_grafana.cmd_help("")
        assert "graph" in result

    async def test_help_excludes_graph_when_not_configured(self, handler):
